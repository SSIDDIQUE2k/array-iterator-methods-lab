...
inventors.filter((year) => {
  if(year.year > 1500 && year.year < 1599){
    veryOldInventors.push(year);
  }
})
...
// Enter your solution code here:
inventors.map((name) => {
  inventorNames.push({first: name.first, last: name.last})
})
...
people.map((name) => {
  ...
  firstLast.push(`${splitName[1]} ${splitName[0]}`)
...
isAdultPresent = inventors.some(inventor => (inventor.passed-inventor.year) >= 18);
